...
public class AddTweetServletUsers extends HttpServlet {

	private static final Logger log = Logger.getLogger(AddTweetServletUsers.class.getName());
			...
			//while(!finish){
		        ...
		        for (int i=0; i<users.length; i++){
			        ...
			        for (int j=0;i<status.size();j++){
			        	...
			        	event.setVerb("tweet");
			        	event.setObject(String.valueOf(status.get(i).getId()));
