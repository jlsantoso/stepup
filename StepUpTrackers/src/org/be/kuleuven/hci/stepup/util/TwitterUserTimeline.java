...
import org.be.kuleuven.hci.stepup.util.rest.RestUtils;
...
public class TwitterUserTimeline {
	...
	private static final Logger log = Logger.getLogger(TwitterUserTimeline.class
			.getName());
	
	private static final String API_URL = "https://api.twitter.com/1.1/statuses/user_timeline.json?";
	...
	private static void testGetTweets() {
		...
		JSONArray result = new TwitterUserTimeline().getTweetsFromHashtag("mume11",since_id);
		int i = 0;
	}
	...
	public JSONArray getTweetsFromHashtag(String hashtag, String since_id) {
		...
		try {
			...
			System.out.println(API_URL + "q=%23" + hashtag +"&rpp=" + RPP + "&since_id=" + since_id + "&include_entities=true");
