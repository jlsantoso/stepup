...
public class CopyInitWeSPOTCourse {
	 ...
	 */
	public static Course wespot(String inquiryId) throws UnsupportedEncodingException, JSONException{
		Course course = new Course();
		int blogs = 9;
		course.setName("wespotdata"+inquiryId);
		...
		String result = RestClient.doPost("http://ariadne.cs.kuleuven.be/wespot-dev-ws/rest/getEvents", "{ \"query\": \"select distinct username from event where context like '%course\":\""+inquiryId+"%'\", \"pag\": \"0\"}");
		System.out.println("Result from: "+"{ \"query\": \"select distinct username from event where context like '%course\":\""+inquiryId+"%'\", \"pag\": \"0\"}");
